...
function TierCard({ tier, examKey }: { tier: CourseTier; examKey: string }) {
    ...
    <Card className="flex flex-col" data-testid={`tier-${tier.name.toLowerCase()}`}>
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-muted-foreground">per {tier.period}</p>
          </div>
        </div>
      ...
      <CardContent className="flex flex-1 flex-col gap-4">
        ...
        <div className="text-xs text-muted-foreground">
          {freeCount} free preview · {lockedCount} locked modules
        </div>
        <Button asChild className="mt-auto w-full">
          <Link href="/sign-up">
            Enroll in {tier.name} <ArrowRight className="ml-2 h-4 w-4" />
          </Link>
        </Button>
      </CardContent>
    </Card>
...
export default async function ExamCoursePage({ params }: Props) {
  ...
  return (
    <SiteShell>
      ...
      <section className="mx-auto max-w-4xl px-4 py-12">
        ...
        <div className="mt-6 flex flex-wrap gap-4 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5">
            ...
            {freeModuleCount} free previews — no sign-in required
          </span>
          <span className="flex items-center gap-1.5">
      ...
      {/* Pricing tiers */}
      <Section title="Choose your tier" subtitle="All tiers include free preview modules. Upgrade or cancel any time.">
        <div className="grid gap-6 md:grid-cols-3">
          {course.tiers.map((tier) => (
            <TierCard key={tier.id} tier={tier} examKey={exam} />
          ))}
        </div>
      </Section>
          ...
          {[
            {
              ...
              a: `Yes. The first ${freeModuleCount} modules across all tiers are free previews — no account needed. Just scroll up and click any "FREE PREVIEW" module.`,
            },
            {
              q: 'What happens after I enroll?',
              a: 'Create a free account, choose your tier, and complete payment via Bkash or card. Access is unlocked immediately.',
            },
            {
              q: 'Can I switch tiers later?',
              a: 'Yes. You can upgrade at any time and pay only the difference. Downgrading takes effect at the next billing cycle.',
            },
            {
              q: 'Is there a refund policy?',
              a: 'If you are not satisfied within the first 7 days, contact us for a full refund — no questions asked.',
            },
          ].map(({ q, a }) => (
      ...
      <section className="mx-auto mb-16 max-w-6xl px-4">
        <div className="rounded-2xl border bg-primary p-10 text-center text-primary-foreground">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm">
            Not sure which tier fits your level? Take the free 5-minute placement test first.
          </p>
